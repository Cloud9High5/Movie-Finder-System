...
import { Box, Button, Chip, Grid, Link, MenuItem, Select, Typography } from '@mui/material';
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={11}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Link variant={'h6'} underline={'hover'} href={''} sx={{ color: 'cadetblue', cursor: 'pointer' }}
                        onClick={() => {
                          path('../movie_detail/' + comment.f_id)
                        }}>
                    {comment.title}
                  </Link>
                  <Typography variant={'p'}>
                    {!helpers.hasNoToken() &&
                    likesDislikes.likes.indexOf(comment.r_id) !== -1 ?
                      <Chip label={'You Liked'} color={'success'} size={'small'} variant={'outlined'}
                            sx={{ marginRight: '10px' }}/> : <></>
                    }

                    {!helpers.hasNoToken() &&
                    likesDislikes.dislikes.indexOf(comment.r_id) !== -1 ?
                ...
                <Box display={'flex'} alignItems={'center'}>
                  <Typography variant={'p'} color={'gray'} onClick={() => {
                    path('/profile/' + comment.u_id)
                  }} sx={{ cursor: 'pointer' }}>
                    {comment.username}
                  </Typography>
                  <Box marginLeft={'10px'}>
